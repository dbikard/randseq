...
# %% auto 0
__all__ = ['plot_motif_analysis', 'plot_motif']
...
from .utils import calculate_log2fc, get_motif_presence_in_library, get_custom_motif_presence_in_library
...
def plot_motif(counts_df, sample_col, ref_col, motif_list, 
                                   left_context_str, right_context_str, 
                                   title_main="Flexible Motif Analysis", pseudocount=1):
    """
    Generates scatterplot for custom user motifs
    """
    motif_presence_df = get_custom_motif_presence_in_library(
            counts_df.index.tolist(), left_context_str, right_context_str, motif_list
        )
    # --- Data Preparation ---
    plot_df = counts_df.copy()
    plot_df['sample_numeric'] = pd.to_numeric(plot_df[sample_col], errors='coerce')
    ...
    for motif in motif_list:
        df_flt = plot_df.loc[motif_presence_df[motif].values]
        ax_scatter_flex.scatter(df_flt['x_plot_values'], df_flt['y_plot_values'], alpha=0.5, s=10, label=motif)
    ax_scatter_flex.loglog()
    ax_scatter_flex.legend()
    plt.show()
